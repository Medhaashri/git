...
def profile_views(config):
    url = f"{config['url']}{config['id']}{config['followers_count_endpoint']}&access_token={config['access_token']}"
    ...
    with io.StringIO() as csv_buffer:
        ...
        redshift.write_data_to_redshift('instagram.followers_count','social-pulse',"instagram/{currentdate}/profile_views.csv".format(currentdate=datetime.today().date()),config['aws_access_key'],config['aws_secret_key'])

    df.to_csv('/Users/medhaashriv/Documents/nbc/followers_count.csv', index=False)
